                ...
                <div class="col-xl-9 col-lg-8">
                    ...
                    <nav class="user-tabs mb-4">
                        <ul class="nav nav-tabs nav-tabs-bottom nav-justified">
                            <li class="nav-item">
                                <a class="nav-link" href="{{ route('freelancers.projects.details', $project->id) }}">Aperçu & Discussions</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="{{ route('freelancers.projects.manage_etape_cles', $project->id) }}">Etapes clés</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="{{ route('freelancers.projects.manage_tasks', $project->id) }}">Tâches</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link active" href="{{ route('freelancers.projects.project_files', $project->id) }}">Fichiers</a>
                            </li>
                        </ul>
                    </nav>
                                ...
                                <div class="table-responsive table-box manage-projects-table">
                                                        ...
                                                        <div class="action-table-data">
                                                            <div class="edit-delete-action m-0">
                                                                ...
                                                                @if ($projectFile->added_by_user_id == auth()->user()->id)
                                                                    <form action="{{ route('projects.files.delete', $projectFile->id) }}" method="POST">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                        <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce fichier?')">Supprimer</button>
                                                                    </form>
                                                                @endif
                                                            </div>
                                                        </div>
                                    ...
                                    @if (count($projectFiles ?? []) == 0)
                                        <div class="text-center text-primary my-5">
                                            ...
                                            <h5 class="mt-1">Aucune donnée n'est spécifiée pour le moment.</h5>
                                        </div>
                                    @endif
                                </div>
                            </div>
